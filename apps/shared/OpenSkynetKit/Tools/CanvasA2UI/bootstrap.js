...
class OpenSkynetA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.openskynetCanvasA2UIAction ??
      globalThis.openskynetCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.openskynetCanvasA2UIAction) {
...
if (!customElements.get("openclaw-a2ui-host")) {
  customElements.define("openclaw-a2ui-host", OpenSkynetA2UIHost);
}
