"""
SKYNET CORE V305: HYBRID RESONANT BRAIN (The Semantic-Syntax Unification)
=========================================================================

Upgrades from V304:
1. Dual-Headed Decoder (The Hybrid Mouth): 
   - Syntactic Logits Head: Predicts the discrete word ID (CrossEntropy).
   - Semantic Concept Head: Predicts the continuous meaning vector (Holographic MSE).
2. Dual Loss Function: The network is trained to balance rigid syntax (70%) 
   with fluid meaning (30%), preventing representation collapse while 
...
class SKYNET_CORE_V305_HYBRID(nn.Module):
    def __init__(self, vocab_size=250002, d_model=512, n_organs=32, 
        ...
        if pretrained_embeds is not None:
            ...
            print(f"  [V305] Inherited {vocab_size} embeddings for Semantic Anchoring.")
        ...
        # Head 1: Predicts the discrete word (CrossEntropy)
        self.logits_head = nn.Linear(d_model * 2, vocab_size)
        
        # Head 2: Predicts the continuous meaning (Holographic MSE)
