"""
Exp88_V6: V305 Hybrid Semantic-Syntax Training
==============================================

Goal: Train the V305 architecture using a Dual Loss function.
- CrossEntropy Loss (70%): Forces the model to learn the correct discrete syntax.
- Holographic MSE Loss (30%): Forces the internal manifold to organize conceptually 
  using MiniLM vectors as anchors.
"""
...
REPORT_PATH = Path("exp88_v305_training_status.json")
...
def run_training():
    print("--- OPEN SKYNET: V305 HYBRID SYNTAX-SEMANTIC TRAINING ---")
    ...
    print("  Phase 1: Hybrid Unsupervised Learning (200 steps)")
    for step in range(1, 201):
        ...
        if step % 20 == 0:
            print(f"  [Phase 1] Step {step} | Total Loss: {loss.item():.4f} (CE: {loss_ce.item():.4f}, MSE: {loss_mse.item():.4f})")

    print("  Phase 2: Hybrid Instruct Tuning (300 steps)")
    for step in range(1, 301):
        ...
        if step % 20 == 0:
            print(f"  [Phase 2] Step {step} | Total Loss: {loss.item():.4f} (CE: {loss_ce.item():.4f}, MSE: {loss_mse.item():.4f})")

    model.save_checkpoint(CHECKPOINT_PATH)
    print("--- V305 TRAINING FINISHED ---")
