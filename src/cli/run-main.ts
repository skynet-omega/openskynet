...
export function rewriteSkynetArgv(argv: string[]): string[] {
  const primary = getPrimaryCommand(argv);
  if (primary === "openskynet") {
    const index = argv.indexOf("openskynet");
    if (index !== -1) {
      const next = [...argv];
      next.splice(index, 1);
      return next;
...
export async function runCli(argv: string[] = process.argv) {
  fs.appendFileSync("openskynet-gateway.log", `RUNNING CLI WITH ARGV: ${JSON.stringify(argv)}\n`);
  ...
  try {
    ...
    const parseArgv = rewriteSkynetArgv(rewriteUpdateFlagArgv(normalizedArgv));
    ...
    fs.appendFileSync("openskynet-gateway.log", `ERROR CAUGHT: ${String(error)}\n${(error as Error).stack}\n`);
    throw error;
  } finally {
