...
const resolveCliNameMock = vi.fn(() => "openclaw");
const resolveCliDisplayNameMock = vi.fn(() => "OpenClaw");
...
vi.mock("../cli-name.js", () => ({
  resolveCliName: resolveCliNameMock,
  resolveCliDisplayName: resolveCliDisplayNameMock,
...
describe("configureProgramHelp", () => {
  ...
  beforeEach(() => {
    ...
    resolveCliNameMock.mockReturnValue("openclaw");
    resolveCliDisplayNameMock.mockReturnValue("OpenClaw");
  });
  ...
  it("prints OpenSkynet when launched via openskynet", () => {
    process.argv = ["node", "openskynet", "--version"];
    resolveCliNameMock.mockReturnValue("openskynet");
    resolveCliDisplayNameMock.mockReturnValue("OpenSkynet");
    expectVersionExit({ expectedVersion: "OpenSkynet 9.9.9-test (abc1234)" });
  });
});
