...
export function buildParseArgv(params: {
  ...
  const normalizedArgv =
    programName && (baseArgv[0] === programName || baseArgv[0].endsWith("openskynet"))
      ? baseArgv.slice(1)
      : (baseArgv[0]?.endsWith("openclaw") || baseArgv[0]?.endsWith("openskynet"))
        ? baseArgv.slice(1)
        : baseArgv;
  const looksLikeNode =
...
export function shouldMigrateStateFromPath(path: string[]): boolean {
  ...
  if (primary === "openclaw" || primary === "openskynet") {
    return true;
  }
