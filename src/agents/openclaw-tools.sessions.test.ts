...
const SESSION_TOOLS_TEST_CONFIG = {
  session: {
    mainKey: "main",
    scope: "per-sender",
    agentToAgent: { maxPingPongTurns: 2 },
...
function createSessionTools(
  ...
  return createOpenClawTools({
    ...options,
    config: options?.config ?? (SESSION_TOOLS_TEST_CONFIG as any),
  });
}
...
describe("sessions tools", () => {
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createSessionTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createSessionTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_send scopes bare main to the requester agent main session", async () => {
    callGatewayMock.mockImplementation(async (opts: unknown) => {
      const request = opts as {
        method?: string;
        params?: Record<string, unknown>;
      ...
      if (request.method === "agent") {
        return { runId: "run-ops-main", acceptedAt: 123 };
      }
      return {};
    });

    const tool = createSessionTools({
      agentSessionKey: "agent:ops:subagent:worker",
      agentChannel: "discord",
    }).find((candidate) => candidate.name === "sessions_send");
    expect(tool).toBeDefined();
    if (!tool) {
