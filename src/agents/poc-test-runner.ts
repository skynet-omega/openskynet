 ...
 *
 * Usage:
 *   pnpm tsx src/agents/poc-test-runner.ts
 *   # or
...
import { testPOC1DynamicTuning, benchmarkPOC1Latency } from "./poc-1-dynamic-tuning.js";
import { testPOC2Grounding, benchmarkPOC2Latency } from "./poc-2-grounding-validator.js";
import { testPOC3CompressedPrompts, testPOC3WithDynamicTuning, benchmarkPOC3Latency } from "./poc-3-compressed-prompts.js";
...
async function runAllTests() {
  ...
  const bestPOC = passedPOCs.reduce((best, current) => {
    ...
    const scoreB = parseInt(current.maxAlucinationReduction) / (current.complexity === "high" ? 2 : 1);
  ...
  console.log(
    `Implement ${bestPOC?.name || "selected POC(s)"} with the following priority:\n`,
  );

  // Sort by impact and ease
  const byPriority = passedPOCs.sort((a, b) => {
    const scoreA = parseInt(a.maxAlucinationReduction) * (a.implementationEffort === "easy" ? 2 : 1);
    const scoreB = parseInt(b.maxAlucinationReduction) * (b.implementationEffort === "easy" ? 2 : 1);
