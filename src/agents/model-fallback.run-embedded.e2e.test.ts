...
const RATE_LIMIT_ERROR_PAYLOAD = '{"type":"error","error":{"type":"rate_limit_error","message":"API rate limit reached. Please try again later."}}';

function mockPrimaryRateLimitThenFallbackSuccessSameProvider() {
  runEmbeddedAttemptMock.mockImplementation(async (params: unknown) => {
    const attemptParams = params as { provider: string; modelId: string; authProfileId?: string };
    if (attemptParams.provider === "openai" && attemptParams.modelId === "mock-1") {
      return makeAttempt({
        assistantTexts: [],
        lastAssistant: buildAssistant({
          provider: "openai",
          model: "mock-1",
          stopReason: "error",
          errorMessage: RATE_LIMIT_ERROR_PAYLOAD,
        }),
      });
    }
    if (attemptParams.provider === "openai" && attemptParams.modelId === "mock-fallback") {
      return makeAttempt({
        assistantTexts: ["fallback ok"],
        lastAssistant: buildAssistant({
          provider: "openai",
    ...
    throw new Error(`Unexpected provider ${attemptParams.provider} / ${attemptParams.modelId}`);
  });
}
...
describe("runWithModelFallback + runEmbeddedPiAgent overload policy", () => {
  ...
  it("falls back to sibling model on same provider when primary hits rate limit", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir);
      mockPrimaryRateLimitThenFallbackSuccessSameProvider();

      const cfg = makeConfig();
      // Override default fallback to use same-provider fallback!
      (cfg.agents!.defaults!.model! as any).fallbacks = ["openai/mock-fallback"];

      const result = await runWithModelFallback({
        cfg,
        provider: "openai",
        model: "mock-1",
        runId: "run:rate-limit-same-provider",
        agentDir,
        run: (provider, model, options) =>
          runEmbeddedPiAgent({
            sessionId: `session:rate-limit-same-provider`,
            sessionKey: "agent:test:rate-limit-same-provider",
            sessionFile: path.join(workspaceDir, `rate-limit-same-provider.jsonl`),
            workspaceDir,
            agentDir,
            config: cfg,
            prompt: "hello",
      ...
      const usageStats = await readUsageStats(agentDir);
      // Because the fallback model succeeded on the same 'openai:p1' profile,
      // the profile's error counters and cooldown were reset by markAuthProfileUsed.
      expect(usageStats["openai:p1"]?.cooldownUntil).toBeUndefined();
      expect(usageStats["openai:p1"]?.failureCounts).toBeUndefined();

      expect(runEmbeddedAttemptMock).toHaveBeenCalledTimes(2);
      const firstCall = runEmbeddedAttemptMock.mock.calls[0]?.[0] as { modelId?: string };
      const secondCall = runEmbeddedAttemptMock.mock.calls[1]?.[0] as { modelId?: string };
      expect(firstCall?.modelId).toBe("mock-1");
      expect(secondCall?.modelId).toBe("mock-fallback");

      expect(computeBackoffMock).toHaveBeenCalledTimes(0); // rate limits might not have backoff, or they might? Wait, rate limit doesn't trigger overload backoff? overload=true, rate_limit doesn't.
    });
  });
