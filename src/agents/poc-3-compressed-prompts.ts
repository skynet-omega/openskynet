...
- Don't over-explain obvious concepts
  ...
  const compressedVersion = generateCompressedSystemPrompt({
    taskDescription:
      "Implement feature following TypeScript strict mode and existing code style",
    outputFormat: "code",
  ...
  console.log(`  Original: ${metrics.originalLength} chars (~${Math.ceil(metrics.originalLength / 4)} tokens)`);
  console.log(`  Compressed: ${metrics.compressedLength} chars (~${Math.ceil(metrics.compressedLength / 4)} tokens)`);
  ...
  console.assert(
    metrics.tokenReduction >= 40,
    "Should achieve at least 40% token reduction",
  );
...
export function testPOC3WithDynamicTuning() {
  ...
  if (contextWindow - estimatedTokens < 1000) {
    console.log(
      "⚠️  Limited context remaining. Enable aggressive pruning of old messages.",
    );
  } else {
...
export function benchmarkPOC3Latency(): void {
  ...
  console.assert(
    avgLatency < 1.0,
    `Expected <1ms per generation, got ${avgLatency.toFixed(4)}ms`,
  );
