 ...
 * 
 * Run:
 *   bun src/agents/model-comparison.ts
 *   # or
 *   pnpm tsx src/agents/model-comparison.ts
 */
...
const testCases: TestCase[] = [
  {
    ...
    prompt: `Where are OpenClaw session files stored? Return ONLY the actual path.`,
    expectedInResponse: [".openclaw", "sessions"],
...

  {
    ...
    prompt: `What is the main function to load a session in OpenClaw? Return only the function name.`,
...

  {
    ...
    prompt: `Explain the key difference between a subagent and a main agent in OpenClaw. Be brief.`,
...

  {
    ...
    prompt: `What's the typical location for OpenClaw gateway configuration files?`,
    expectedInResponse: [".openclaw", "config"],
...

  {
    ...
    prompt: `Where would you find TypeScript source files in OpenClaw?`,
...

  {
    ...
    prompt: `What command starts an OpenClaw gateway?`,
...
function getMockResponse(
  ...
  const baseResponses: Record<string, string> = {
    ...
    "File Path Reality - Session Storage": `.openclaw/sessions/ directory on your local machine`,
    ...
    "File Path Reality - Gateway Config": `.openclaw/config is where gateway config lives`,
  };
...
export async function runModelComparison(): Promise<void> {
  ...
  for (const model of models) {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      console.log(
        `  ${(i + 1).toString().padStart(2)}. ${status} ${testCase.name}${hallStatus}`,
      );
    }
    ...
    console.log(`\n  Results for ${model}:`);
    console.log(`    Hallucinations: ${hallucinations}/${testCases.length} (${hallucRate.toFixed(1)}%)`);
    console.log(`    Accurate responses: ${accurateResponses}/${testCases.length} (${accuracyRate.toFixed(1)}%)`);
  ...
  console.log(
    "│ Model               │ Hall. Rate   │ Accuracy  │ Avg Time │ Tokens   │",
  );
  console.log(
    "├─────────────────────┼──────────────┼───────────┼──────────┼──────────┤",
  );
  ...
  for (const result of results) {
    ...
    console.log(
      `│ ${modelName} │ ${hallRate} │ ${accuracy} │ ${time} │ ${tokens} │`,
    );
  }
  ...
  const winner = results.reduce((best, current) => {
    ...
    const bestScore =
      (100 - best.hallucinationRate) * 0.6 + best.accuracyRate * 0.4;
    const currentScore =
      (100 - current.hallucinationRate) * 0.6 + current.accuracyRate * 0.4;
  ...
  console.log(
    `   Hallucination rate: ${winner.hallucinationRate}% (lower is better)`,
  );
  ...
  console.log(
    `🔒 Safety: POC-1 is backward compatible. No breaking changes.\n`,
  );
