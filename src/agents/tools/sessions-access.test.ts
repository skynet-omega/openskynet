...
describe("createSessionVisibilityGuard", () => {
  ...
  it("allows descendants beyond direct children when visibility=tree", async () => {
    const cfg = {
    ...
    expect(guard.check("agent:main:subagent:leaf")).toEqual({ allowed: true });
    expect(guard.check("agent:main:subagent:sibling")).toEqual({
      allowed: false,
      status: "forbidden",
      error:
