...
import {
  buildOmegaInteractionPrompt,
  buildOmegaSessionContextPrompt,
  runValidatedOmegaSessionTask,
} from "../../omega/index.js";
import type { OmegaEmpiricalRoute } from "../../omega/empirical-metrics.js";
...
export function createSessionsSendTool(opts?: {
  ...
  executionRoute?: OmegaEmpiricalRoute;
}): AnyAgentTool {
  return {
    label: "Session Send",
    name: "sessions_send",
    ...
    execute: async (_toolCallId, args) => {
      ...
      const timeoutSeconds =
        typeof params.timeoutSeconds === "number" && Number.isFinite(params.timeoutSeconds)
          ...
          : 30;
      const { expectsJson, expectedKeys, expectedPaths, requiresValidation } =
        readOmegaValidationToolParams(params);
      if (timeoutSeconds === 0 && requiresValidation) {
        return jsonResult({
          runId: crypto.randomUUID(),
          status: "error",
          errorKind: "validation_requires_wait",
          error: "Validation requires timeoutSeconds > 0.",
      ...
      const omegaSessionContextPrompt = await buildOmegaSessionContextPrompt({
      ...
      const omegaValidationPrompt = buildOmegaValidationPrompt({
        expectedKeys,
        expectedPaths,
      });
      const extraSystemPrompt = [
        agentMessageContext,
        omegaSessionContextPrompt,
        omegaInteractionPrompt,
        omegaValidationPrompt,
      ]
      ...
      const deliveryMode = opts?.deliveryMode === "none" ? "none" : "announce";
      const delivery =
        deliveryMode === "none"
      ...
      const execution = await runValidatedOmegaSessionTask({
        ...
        execution: {
          route: opts?.executionRoute ?? "sessions_send",
          trigger: "direct",
        },
      });
      ...
      if (!execution.ok) {
        return jsonResult({
          ...
          observedChangedFiles: execution.observedChangedFiles,
      ...
      startA2AFlow(execution.reply ?? undefined);
