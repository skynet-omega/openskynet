...
export function createSessionsSpawnTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const extraSystemPrompt = [omegaInteractionPrompt, omegaValidationPrompt]
        .filter((value): value is string => typeof value === "string" && value.trim().length > 0)
        .join("\n\n");
      const thread = params.thread === true;
      const resolvedMode = mode ?? (thread ? "session" : "run");
      if (requiresValidation && resolvedMode !== "run") {
        return jsonResult({
          status: "error",
          errorKind: "validation_requires_run_mode",
          error: 'Validation requires mode="run" (thread-bound sessions stay persistent).',
        });
      }
      if (requiresValidation && (!runTimeoutSeconds || runTimeoutSeconds <= 0)) {
        return jsonResult({
          status: "error",
          errorKind: "validation_requires_timeout",
          error: "Validation requires runTimeoutSeconds > 0.",
        });
      }
      ...
      if (
        ...
        const execution = await awaitValidatedOmegaSessionRun({
          ...
          timeoutMs: (runTimeoutSeconds ?? 0) * 1000,
          workspaceRoot: opts?.workspaceDir ?? process.cwd(),
          validation: {
            expectsJson,
            expectedKeys,
