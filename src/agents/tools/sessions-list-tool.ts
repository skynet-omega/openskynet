  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const list = await callGateway<{ sessions: Array<SessionListRow>; path: string }>({
        ...
        params: {
          limit: restrictToSpawned ? undefined : limit,
          activeMinutes,
          includeGlobal: !restrictToSpawned,
          includeUnknown: !restrictToSpawned,
          agentId: restrictToSpawned ? resolveAgentIdFromSessionKey(effectiveRequesterKey) : undefined,
        },
      });
      ...
      if (limit !== undefined && rows.length > limit) {
        rows.length = limit;
        historyTargets.length = Math.min(historyTargets.length, limit);
      }
