  ...
  it("maps alias and main key to display main", () => {
    ...
    expect(
      resolveDisplaySessionKey({
        ...
        alias: "main",
        mainKey: "main",
        requesterSessionKey: "agent:ops:subagent:worker",
      }),
    ).toBe("main");
  });
...
describe("resolved session visibility checks", () => {
  ...
  it("treats deep descendants as visible for spawned-session checks", async () => {
    const cfg = {
    ...
    await expect(
      isResolvedSessionVisibleToRequester({
        requesterSessionKey: "agent:main:main",
        targetSessionKey: "agent:main:subagent:leaf",
        restrictToSpawned: true,
        resolvedViaSessionId: false,
        cfg,
        store,
