...
function formatRateLimitOrOverloadedErrorCopy(raw: string, provider?: string): string | undefined {
  if (isRateLimitErrorMessage(raw)) {
    // If it's a local model (ollama), or if it looks like a specific resource limit, show the raw error.
...
export function formatAssistantErrorText(
  ...
  // Skip generic formatting for multi-model fallback summaries to avoid masking individual failures
