...
describe("formatRawAssistantErrorForUi", () => {
  it("rewrites periodic usage limit errors into a clear quota message", () => {
    expect(
      formatRawAssistantErrorForUi(
        '429 Too Many Requests: you have reached your weekly usage limit, please wait or upgrade to continue',
      ),
    ).toBe(
      "⚠️ Model usage limit reached for the current period. Wait for quota reset or switch to another model.",
    );
  });

  it("rewrites timeout errors into a short actionable message", () => {
    expect(formatRawAssistantErrorForUi("network timeout after 30000ms")).toBe(
      "LLM request timed out. Try again, reduce the task size, or switch to a faster model.",
    );
  });

  it("rewrites generic rate limit payloads into a concise user-facing message", () => {
    ...
    expect(text).toBe("⚠️ API rate limit reached. Please try again later.");
  });
