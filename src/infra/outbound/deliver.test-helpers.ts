...
const _hookMocks = vi.hoisted(() => ({
  runner: {
    ...
    runMessageSent: vi.fn(
      async (...args: unknown[]) => await (deliverMocks.hooks.runner.runMessageSent as any)(...args),
    ),
  },
}));
const _internalHookMocks = vi.hoisted(() => ({
  createInternalHookEvent: vi.fn((...args: unknown[]) =>
    (deliverMocks.internalHooks.createInternalHookEvent as any)(...args),
  ),
  triggerInternalHook: vi.fn(
    async (...args: unknown[]) => await (deliverMocks.internalHooks.triggerInternalHook as any)(...args),
  ),
}));
const _queueMocks = vi.hoisted(() => ({
  enqueueDelivery: vi.fn(
    async (...args: unknown[]) => await (deliverMocks.queue.enqueueDelivery as any)(...args),
  ),
  ackDelivery: vi.fn(async (...args: unknown[]) => await (deliverMocks.queue.ackDelivery as any)(...args)),
  failDelivery: vi.fn(async (...args: unknown[]) => await (deliverMocks.queue.failDelivery as any)(...args)),
}));
const _logMocks = vi.hoisted(() => ({
  warn: vi.fn((...args: unknown[]) => (deliverMocks.log.warn as any)(...args)),
}));
...
export async function runChunkedWhatsAppDelivery(params: {
  ...
  mirror?: unknown;
}): Promise<{ sendWhatsApp: any; results: Array<{ messageId?: string; toJid?: string }> }> {
