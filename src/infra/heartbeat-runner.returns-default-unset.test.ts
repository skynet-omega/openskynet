...
describe("runHeartbeatOnce", () => {
  ...
  it("injects OMEGA wake guidance into heartbeat prompts when follow-up is not directly resumable", async () => {
    const tmpDir = await createCaseDir("hb-omega-wake");
    const storePath = path.join(tmpDir, "sessions.json");
    const replySpy = vi.spyOn(replyModule, "getReplyFromConfig");
    const sendWhatsApp = vi
      .fn<NonNullable<HeartbeatDeps["sendWhatsApp"]>>()
    ...
    try {
      ...
      const sessionKey = resolveMainSessionKey(cfg);
      await fs.writeFile(path.join(tmpDir, "HEARTBEAT.md"), "- Review scientific state\n", "utf-8");
      await fs.writeFile(
        storePath,
        JSON.stringify({
          [sessionKey]: {
      ...
      await recordOmegaSessionOutcome({
        ...
        sessionKey,
        task: "corrige la falla intermitente",
        validation: {
          expectsJson: false,
          expectedKeys: [],
          expectedPaths: [],
      ...
      expect(res.status).toBe("ran");
      const calledCtx = replySpy.mock.calls[0]?.[0] as { Body?: string };
      expect(calledCtx.Body).toContain("[OMEGA Wake]");
      expect(calledCtx.Body).toContain("Wake action: review_active_goal");
      expect(calledCtx.Body).toContain("Active goal requiring follow-up: corrige la falla intermitente");
    } finally {
      replySpy.mockRestore();
    }
  });

  it("aborts repeated interrupted-goal recovery without calling the model", async () => {
    const tmpDir = await createCaseDir("hb-omega-abort");
    const storePath = path.join(tmpDir, "sessions.json");
    const replySpy = vi.spyOn(replyModule, "getReplyFromConfig");
    const sendWhatsApp = vi
      .fn<NonNullable<HeartbeatDeps["sendWhatsApp"]>>()
    ...
    try {
      ...
      const kernel = await loadOmegaSelfTimeKernel({ workspaceRoot: tmpDir, sessionKey });
      expect(kernel?.tension.failureStreak).toBe(2);
    } finally {
      replySpy.mockRestore();
    }
  });

  it("executes stale-goal maintenance without calling the model", async () => {
    const tmpDir = await createCaseDir("hb-omega-exec");
    const storePath = path.join(tmpDir, "sessions.json");
    const replySpy = vi.spyOn(replyModule, "getReplyFromConfig");
    const sendWhatsApp = vi
      .fn<NonNullable<HeartbeatDeps["sendWhatsApp"]>>()
    ...
    try {
      ...
      expect(sendWhatsApp).not.toHaveBeenCalled();
      const kernel = await loadOmegaSelfTimeKernel({ workspaceRoot: tmpDir, sessionKey });
      expect(kernel?.goals.some((goal) => goal.task === "task A")).toBe(false);
      expect(kernel?.goals.some((goal) => goal.task === "task B")).toBe(true);
    } finally {
      replySpy.mockRestore();
    }
  });

  it("executes superseded-goal maintenance without calling the model", async () => {
    const tmpDir = await createCaseDir("hb-omega-superseded");
    const storePath = path.join(tmpDir, "sessions.json");
    const replySpy = vi.spyOn(replyModule, "getReplyFromConfig");
    const sendWhatsApp = vi
      .fn<NonNullable<HeartbeatDeps["sendWhatsApp"]>>()
    ...
    try {
      ...
      expect(sendWhatsApp).not.toHaveBeenCalled();
      const kernel = await loadOmegaSelfTimeKernel({ workspaceRoot: tmpDir, sessionKey });
      expect(kernel?.goals.some((goal) => goal.task === "fix module and test together")).toBe(false);
      expect(kernel?.goals.some((goal) => goal.task === "fix module only")).toBe(true);
      expect(kernel?.goals.some((goal) => goal.task === "fix test only")).toBe(true);
    } finally {
      replySpy.mockRestore();
    }
  });

  it("focuses active-goal targets after verified partial progress without calling the model", async () => {
    const tmpDir = await createCaseDir("hb-omega-focus");
    const storePath = path.join(tmpDir, "sessions.json");
    const replySpy = vi.spyOn(replyModule, "getReplyFromConfig");
    const sendWhatsApp = vi
      .fn<NonNullable<HeartbeatDeps["sendWhatsApp"]>>()
    ...
    try {
      ...
      expect(replySpy).not.toHaveBeenCalled();
      expect(sendWhatsApp).not.toHaveBeenCalled();
      const state = await loadOmegaSessionSelfState({ workspaceRoot: tmpDir, sessionKey });
      expect(state?.activeTargets).toEqual(["workspace/b.py"]);
    } finally {
      replySpy.mockRestore();
    }
  });

  it("prunes shadowed goals without calling the model", async () => {
    const tmpDir = await createCaseDir("hb-omega-shadowed");
    const storePath = path.join(tmpDir, "sessions.json");
    const replySpy = vi.spyOn(replyModule, "getReplyFromConfig");
    const sendWhatsApp = vi
      .fn<NonNullable<HeartbeatDeps["sendWhatsApp"]>>()
    ...
    try {
      ...
      const state = await loadOmegaSessionSelfState({ workspaceRoot: tmpDir, sessionKey });
      expect(kernel?.goals.some((goal) => goal.task === "fix module and test together")).toBe(false);
      expect(state?.activeGoal).toBe("fix test only");
      expect(state?.activeTargets).toEqual(["workspace/b.py"]);
    } finally {
      replySpy.mockRestore();
    }
