...
import {
  POSIX_OPENSKYNET_TMP_DIR,
  resolvePreferredOpenSkyNetTmpDir,
} from "./tmp-openskynet-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOpenSkyNetTmpDir>[0]>;
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredOpenSkyNetTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_OPENSKYNET_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOpenSkyNetTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredOpenSkyNetTmpDir", () => {
  it("prefers /tmp/openskynet when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPENSKYNET_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/openskynet when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENSKYNET_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/openclaw when /tmp/openskynet is not a directory", () => {
  ...
  it("falls back when /tmp/openskynet exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_OPENSKYNET_TMP_DIR) {
  ...
  it("falls back when /tmp/openskynet is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/openskynet is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/openskynet is group/other writable", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ mode: 0o40777 })) });
  });

  it("repairs existing /tmp/openskynet permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_OPENSKYNET_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_OPENSKYNET_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_OPENSKYNET_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/openskynet after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_OPENSKYNET_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_OPENSKYNET_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENSKYNET_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_OPENSKYNET_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback OpenSkynet temp dir/);
  });
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredOpenSkyNetTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_OPENSKYNET_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback OpenSkynet temp dir/);
  });
});
