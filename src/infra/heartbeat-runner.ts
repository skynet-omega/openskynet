...
async function appendOmegaHeartbeatHint(params: {
  prompt: string;
  workspaceDir: string;
  sessionKey: string;
}): Promise<string> {
  const omegaPrompt = await buildOmegaHeartbeatPrompt({
...
export async function runHeartbeatOnce(opts: {
  ...
  const omegaExecutive = await applyOmegaHeartbeatExecutiveAction({
    workspaceRoot: workspaceDir,
    sessionKey,
    requesterAgentIdOverride: agentId,
  });
  if (
    omegaExecutive.kind === "pruned_stale_goals" ||
    omegaExecutive.kind === "pruned_superseded_goals" ||
    ...
    const previewBody =
      omegaExecutive.kind === "focused_active_goal_targets"
        ...
        : omegaExecutive.kind === "aborted_interrupted_goal"
          ? [
              omegaExecutive.goalTask,
              `failure_streak=${omegaExecutive.failureStreak}`,
              omegaExecutive.errorKind,
            ]
    ...
    emitHeartbeatEvent({
      ...
      preview: `${previewPrefix}: ${previewBody}`.slice(0, 200),
      durationMs: Date.now() - startedAt,
      channel: delivery.channel !== "none" ? delivery.channel : undefined,
      accountId: delivery.accountId,
      silent: true,
  ...
  const resolvedPrompt = await appendOmegaHeartbeatHint({
    prompt,
    workspaceDir,
    sessionKey,
  });
  const ctx = {
    Body: appendCronStyleCurrentTimeLine(resolvedPrompt, cfg, startedAt),
    From: sender,
    To: sender,
