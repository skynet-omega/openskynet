...
export function enhanceDriveWithJepaTension(
  ...
  if (driveSignal.kind === "idle" && jepaTension.frustration > 1.5) {
    return {
      kind: "entropy_alert",
      silentMs: 0, // JEPA triggered, no es silencio temporal
      reason: `JEPA-EMERGENCY: frustration=${jepaTension.frustration.toFixed(2)} (>1.5 threshold)`,
  ...
  if (driveSignal.kind !== "idle" && jepaTension.frustration > 0.5) {
    ...
    // Escalera: frustration 0.5 → +0.075 boost, frustration 1.5 → +0.225 boost
    ...
    return {
      ...driveSignal,
      urgency: newUrgency,
      reason: `${driveSignal.reason} [JEPA-ENHANCED: +${(boostAmount * 100).toFixed(0)}%]`,
    };
  }

  // Caso 3: Idle drive con moderate frustration (0.5-1.5)
  // Don't activate entropy_alert yet, but prepare higher urgency
  if (driveSignal.kind === "idle" && jepaTension.frustration > 0.5 && jepaTension.frustration <= 1.5) {
    ...
    const urgency = 0.4 + jepaTension.frustration * 0.15;

    if (nextDrive === "curiosity") {
      return {
        kind: "curiosity",
        target: "memory/",
        reason: `JEPA-ENGAGED: moderate frustration=${jepaTension.frustration.toFixed(2)}`,
        urgency: Math.min(0.8, urgency),
      };
    } else {
      return {
        kind: "homeostasis",
...
export function parseJepaTensionFromKernelTimeline(
  ...
  // Contar fallos en últimos 5 turns (ventana reciente)
  ...
  // Frustration = failureRate * 2 (0-2 scale)
  // confidence = 1 - failureRate (0-1 scale)
  return {
