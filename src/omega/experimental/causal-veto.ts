...
export type VetoFeatureVector = [number, number, number, number, number];

export class PredictiveVetoEngine {
  weights: number[];
  bias: number;
  learningRate: number;
  private statePath: string;

  constructor(workspaceRoot: string, numFeatures: number = 5) {
    this.weights = new Array(numFeatures).fill(0);
    this.bias = 0;
    this.learningRate = 0.1;
    this.statePath = path.join(workspaceRoot, ".openskynet", "causal-veto-weights.json");
  }
  ...
  predict(features: VetoFeatureVector): number {
    let sum = this.bias;
  ...
  async update(features: VetoFeatureVector, actualOutcome: number): Promise<void> {
