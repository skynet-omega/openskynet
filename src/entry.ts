...
const ENTRY_WRAPPER_PAIRS = [
  ...
  { wrapperBasename: "openskynet.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "openskynet.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const cliName = resolveCliName(process.argv);
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `${cliDisplayName} ${VERSION} (${commit})` : `${cliDisplayName} ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!tryHandleRootVersionFastPath(process.argv) && !tryHandleRootHelpFastPath(process.argv)) {
      ...
      import("./cli/run-main.js")
        .then(({ runCli }) => runCli(currentArgv))
        .catch((error) => {
          console.error(
