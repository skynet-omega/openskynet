...
const resolveCliDisplayNameMock = vi.hoisted(() => vi.fn((argv?: string[]) =>
  argv?.[1]?.includes("openskynet") ? "OpenSkynet" : "OpenClaw",
));
const resolveCliNameMock = vi.hoisted(() => vi.fn((argv?: string[]) =>
  argv?.[1]?.includes("openskynet") ? "openskynet" : "openclaw",
));
...
describe("entry root version fast path", () => {
  ...
  it("prints OpenSkynet when launched via openskynet", async () => {
    ...
    await vi.waitFor(() => {
      expect(logSpy).toHaveBeenCalledWith("OpenSkynet 9.9.9-test (abc1234)");
      expect(exitSpy).toHaveBeenCalledWith(0);
    });
