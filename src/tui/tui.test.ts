...
import {
  ...
  resolvePreferredTuiFallbackAgentId,
...
describe("resolvePreferredTuiFallbackAgentId", () => {
  const cfg: OpenClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/openclaw" },
        { id: "openskynet", workspace: "/tmp/openskynet" },
      ],
    },
  };

  it("prefers the openskynet agent when launched via openskynet", () => {
    expect(
      resolvePreferredTuiFallbackAgentId({
        cfg,
        fallbackAgentId: "main",
  ...
  it("keeps the configured fallback for openclaw", () => {
    expect(
      resolvePreferredTuiFallbackAgentId({
        cfg,
        fallbackAgentId: "main",
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    process.argv = ["node", "openskynet", "tui"];
    ...
    expect(state.activityStatus).toBe("pairing required: run openskynet devices list");
    expect(state.pairingHint).toContain("openskynet devices list");
  });
