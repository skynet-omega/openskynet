...
export function resolvePreferredTuiFallbackAgentId(params: {
  cfg: OpenClawConfig;
  fallbackAgentId: string;
  cliName?: string;
}) {
  const fallbackAgentId = normalizeAgentId(params.fallbackAgentId);
  const cliName = params.cliName ?? resolveCliName();
  ...
  return preferredAgentId ? normalizeAgentId(preferredAgentId) : fallbackAgentId;
}
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: `pairing required: run ${cliName} devices list`,
      pairingHint: `Pairing required. Run \`${cliName} devices list\`, approve your request ID, then reconnect.`,
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  let agentDefaultId = resolvePreferredTuiFallbackAgentId({
    cfg: config,
  ...
  // Daemon coordination: Create interaction lock
  ...
  try {
    ...
    // Continue even if lock creation fails
  }
  ...
  const requestExit = async () => {
    ...
    // Exit with code 0 (allows pending promises to resolve)
    process.exitCode = 0;
  };
